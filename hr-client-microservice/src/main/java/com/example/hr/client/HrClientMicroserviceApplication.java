...
public class HrClientMicroserviceApplication implements ApplicationRunner {
	...
	public HrClientMicroserviceApplication(RestTemplate restTemplate, DiscoveryClient discoveryClient) {
...
/*		
		...
		this.instances.forEach(instance -> System.err
				.println("Hr Instance\t: %s:%d".formatted(instance.getHost(), instance.getPort())));
		var firstInstance = this.instances.get(0);
		var hireEmployeeRequest = new HireEmployeeRequest();
		hireEmployeeRequest.setTcKimlikNo(tcKimlikNo);
		...
		var response = restTemplate.postForEntity(url.formatted(firstInstance.getHost(), firstInstance.getPort()),
				hireEmployeeRequest, String.class).getBody();
